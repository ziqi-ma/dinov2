...
def global_aug(xyz, rgb, normal):
    ...
    data_dict = RandomDropout(dropout_ratio=0.2,dropout_application_ratio=0.2)(data_dict)
...
def collate_fn(batch):
    ...
    if isinstance(batch[0], Mapping) and "global_aug1" in batch[0] and "global_aug2" in batch[0]:
        flattened_batch = [item["global_aug1"] for item in batch] + [item["global_aug2"] for item in batch]
...
class ObjaverseAugmented(data.Dataset):
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        return {"global_aug1":point_global_aug1, "global_aug2":point_global_aug2}
